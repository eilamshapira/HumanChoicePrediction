...
class MainNetTransformer(nn.Module):
    def __init__(self, n_layers, hidden_dim, include_bot_vector, dropout, nhead):
        super().__init__()
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=self.hidden_dim, nhead=nhead, dropout=dropout)
        self.main_task = nn.TransformerEncoder(self.encoder_layer, num_layers=self.n_layers).double()
    ...
    def forward(self, game):
        ...
        for i in range(DATA_ROUNDS_PER_GAME):
            time_output = self.main_task(lstm_input[:, :i+1].contiguous())[:, -1, :]
